...
const additionButton = document.getElementById("plus-sign");
const subtractionButton = document.getElementById("sub-sign");
const multiplicationButton = document.getElementById("mult-sign");
const divisionButton = document.getElementById("div-sign");
...
document.addEventListener("keyup", (event) =>{
    ...
    if(event.key === "Backspace"){
        ...
        // Highlight the operation buttons in the UI when clicked
        if(event.key === "+") additionButton.style.filter = "brightness(1.16)";
        if(event.key === "-") subtractionButton.style.filter = "brightness(1.16)";
        if(event.key === "*") multiplicationButton.style.filter = "brightness(1.16)";
        if(event.key === "/") divisionButton.style.filter = "brightness(1.16)";
        decimalButton.disabled = false;
        decimalButton.removeAttribute("style");
        clearDisplayFlag = true;
        ...
        if(event.key !== "=" || event.key !== "Enter"){
            operationArray.splice(operationArray.length, 0, event.key);
